...
namespace Pet_Shop.Services
{
    public class ProductService
    {
            ...
            return await _context.Products
                ...
                .Where(p => p.IsActive &&
                            (
                                ...
                                (p.Brand != null && (p.Brand.BrandName ?? "").Contains(searchTerm))
                            ))
                .OrderBy(p => p.ProductName)
                .ToListAsync();
        }
        ...
        {
            // NOTE: OrderBy(Guid.NewGuid()) translates to NEWID() on SQL Server (OK for small sets).
            return await _context.Products
                .AsNoTracking()
                .Include(p => p.Category)
                .Include(p => p.Brand)
        ...
        {
            // Tracking is fine for admin editing screens
            return await _context.Products
                .Include(p => p.Category)
                .Include(p => p.Brand)
                .Include(p => p.ProductImages)
        ...
        /// <summary>
        /// Resolve a list of product codes to Product entities (includes images, brand, category),
        /// preserving the input order. Case-insensitive distinct on input codes.
        /// </summary>
        public async Task<List<Product>> GetProductsByCodesAsync(
            IEnumerable<string> productCodes,
            CancellationToken cancellationToken = default)
        {
            var codeList = (productCodes ?? Enumerable.Empty<string>())
            ...
            // Fetch matches (active products preferred for homepage)
            var products = await _context.Products
                .AsNoTracking()
                .Include(p => p.ProductImages)
                .Include(p => p.Category)
                .Include(p => p.Brand)
                .Where(p => p.IsActive && p.ProductCode != null && codeList.Contains(p.ProductCode))
            ...
            products = products
                .OrderBy(p => order.TryGetValue(p.ProductCode ?? string.Empty, out var idx) ? idx : int.MaxValue)
                .ToList();
