    ...
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                ...
                // ============= AI Recs based on cart items =============
                IEnumerable<Product> cartRecommendations = Enumerable.Empty<Product>();
                try
                {
                    ...
                    if (productIds.Any())
                    {
                        // Bạn điều chỉnh tên hàm theo service của bạn
                        cartRecommendations = await _localRecommendationService
                            .GetCartBasedRecommendationsAsync(userId, productIds, 6);
                    }
                }
                catch (Exception recEx)
                {
                    _logger.LogWarning(recEx,
                        "Error getting cart-based recommendations for user {UserId}", userId);
                }

                ViewBag.CartRecommendations = cartRecommendations;
